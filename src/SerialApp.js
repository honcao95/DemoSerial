...
class SerialApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      servisStarted: false,
  ...
  onReadData(data) {
    var strData = data.payload.slice(14,38);
    var strLengthData = data.payload.length;
    var arr = [];
    // console.log("gjhfhf",strData, strLengthData)
    ...
    if (strLengthData < 42){
      strData = "";
      console.log(strData)
      this.setState({ output: this.state.output + strData + "\n"});
      // this.props.dataActions.postData(strData)
    }else {
      arr.push(strData);
      console.log('data', arr)
      this.setState({ output: this.state.output + arr + "\n"});
      this.props.dataActions.postData(strData)
      arr = [];
  ...
  handleConvertButton() {
    // console.log("herer")
    // console.log(typeof(this.string2Bin(this.state.sendText)))
    var str = "Hello";
    RNSerialport.writeStrToBytes(str)
    // let data = "";
    // if (
      ...
      <ScrollView style={styles.body}>
        <View style={styles.container}>
          ...
          <View style={styles.line2}>
            ...
            >
              <Text style={styles.buttonText}>Get Buffer</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
...
export default connect(mapState, mapDispatch)(SerialApp);
